...
export async function getCurrentWeatherByCoords(coords) {
  const response = await axios.get(`https://api.openweathermap.org/data/2.5/weather?lat=${coords.latitude}&lon=${coords.longitude}&appid=${apiKey}&units=imperial`);
  if (response.status !== 200) {
    throw new Error('You must type an existent city name');
  }
  return response.data;
...
export async function getForecastedTemperaturesByCoords(coords) {
  const response = await axios.get(`https://api.openweathermap.org/data/2.5/forecast?lat=${coords.latitude}&lon=${coords.longitude}&appid=${apiKey}&units=imperial`);
  if (response.status !== 200) {
    throw new Error('You must type an existent city name');
  }
  return response.data;
...
export async function getForecastedDailyWeatherByCity(city) {
  const firstResponse = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiKey}&units=imperial`);
